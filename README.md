...
```bash
npm install
cp .env.example .env    # 用 VS Code 编辑，保存为 UTF-8（无 BOM）！
npm start               # ⚠️ 首次联调用 start，不要用 dev（见下）
```

> **为什么首跑不用 `npm run dev`**：dev 的文件监视器在风控退出（exit 2）后不会退出且无提示——终端看似挂死；且等待期间**保存任何代码文件都会触发自动重启 = 一次全新登录**，加重风控。等服务跑稳（`test-login.mjs` 两域都 ✓）后再切 dev。
...
**Q：`.mi.json` 损坏/内容异常**
→ 直接删除该文件重启即可（会重新登录一次；风控敏感期请确认是正常的登录失败再删）。
