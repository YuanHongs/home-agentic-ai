...
>
> **务必同时配置重启退避**（systemd `RestartSec=60`；Docker 需在入口脚本 sleep）：进程内重登已节流为 5 分钟一次，但每次进程重启本身就是一次登录——不设退避的话，慢性故障下仍是高频登录（风控风险）。
...
1. **报"已触发小米账号安全验证"**：异地登录风控。程序已自动识别并停止重试（exit code 2）——按日志中的授权链接完成验证（见下方"安全验证"条目），等约 1 小时后再启动。**期间勿反复重启**：每次重试都以新设备身份登录，只会加重风控。
2. **`MI_USER_ID` 填了手机号**：要数字 ID（获取方式见第 2 节），不是手机号或邮箱。
3. **`MI_DID` 错误**：与米家 App 里的设备对不上（注意错别字、空格和大小写）。

普通凭证错误启动最多尝试 5 次（5s→40s 退避）；风控错误不重试、直接退出。

**Q：什么是安全验证 / 如何授权**
→ 检测到"异地登录"时小米要求网页验证账号。打开日志里打印的授权链接（`account.xiaomi.com`），登录后按提示完成验证即可；**xiaomiio 和 micoapi 两个域可能各出一条链接，要分别授权**。验证后约 1 小时账号信息才更新，期间请勿重试。拿不准可先跑 `node --env-file=.env test-login.mjs` 单独验证两个域的登录状态。
...
**Q：服务自己退出了**
→ 两种情况：① 连续 10 次重登失败（长期断网/改了密码）会主动退出——这是设计行为（fail-fast），恢复网络/密码后重启，或配进程管理器自动拉起；② 日志出现"已触发小米账号安全验证"——运行期撞上风控也会立即退出（重试无意义，见上文"安全验证"条目），完成授权并等待约 1 小时后再启动。

**Q：想换触发词/全接管**
→ 改 `TRIGGER_WORDS` 后重启。全接管模式注意：所有对话（包括"开灯"这种原生秒回的指令）都会走 1~2 秒 LLM 链路。

> 首次启动需出网访问 miot-spec.org 拉一次设备规范全量列表（约 3MB，10 秒超时）；失败有日志且不阻塞启动，该情况下设备暂无精细控制能力。

## 8. 测试与开发

```bash
npm test            # 160 个单元测试（全部 mock，不联网）
...
- 风控还有一种验证码形态：若日志显示 `👉 undefined`，说明库输出缺陷丢了链接——运行 `node --env-file=.env test-login.mjs`，从错误 JSON 里找 `captchaUrl` 手动打开。
